...
def get_num_instances_per_file(f_name):
    """
    ...
    return np.maximum(1, int(np.ceil((file_frames - 100) / 50)))
...
clean_list_ori = [filelist_audio_tr[i].replace('.wav', suffix_in + '.data') for i in idx_flagveri]
file_list = list(set(te_files) - set(clean_list_ori))

for iteration in range(5):
    # to store predictions
    te_preds = np.empty((len(file_list), 10))
    list_preds = []
    model_list = []
...
#             model_list.append(model)
    ...
    te_gen_patch = DataGeneratorFileFeatures(feature_dir=params_path.get('featurepath_tr'),
                                 file_list=file_list,
                                 params_learn=params_learn,
                                 params_extract=params_extract,
                                 suffix_in='_mel',
    ...
    start_idx = 0
    for i in range(len(te_gen_patch.nb_iterations)):
        print('%d / %d batch' %(i, len(te_gen_patch.nb_iterations)))
        ...
        for model_j in range(10):
            ...
            # predicting now on the T_F patch level (not on the wav clip-level)
            preds_patch_list = model.predict(features).tolist()
            preds_patch = np.array(preds_patch_list)
            te_preds[start: min(start+64, len(te_files)), model_j] = preds_patch
        start_idx += 64
    ...
    train_csv_clean = pd.read_csv(file_name)
    binary_labels = np.array(train_csv_clean[str(target_label)])
    binary_labels[additional_data_index_valid] = 1
