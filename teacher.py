...
train_csv = pd.read_csv('record/generate_clean_data/thres_0.90_crit_9/teacher.csv')
...
if params_ctrl.get('train_data') == 'all':
    ff_list_tr = [filelist_audio_tr[i].replace('.wav', suffix_in + '.data') for i in range(len(filelist_audio_tr))]
...
#     # callbacks
#     early_stop = EarlyStopping(monitor='acc', patience=params_learn.get('patience'), min_delta=0.001, verbose=1)
#     reduce_lr = ReduceLROnPlateau(monitor='acc', factor=0.5, patience=5, verbose=1)
...
# f = open(record_path+'lr_%.5f.txt'%params_learn.get('lr'), 'a+')
# f.write('epoch, loss, training accuracy')
# loss = hist.history['loss']
# acc = hist.history['accuracy']
# for epoch in range(params_learn.get('n_epochs')):
#     f.write(str(epoch)+str(loss[epoch])+str(acc[epoch])+'\n')
# f.close()
# # ==================================================================================================== PREDICT
# # ==================================================================================================== PREDICT
# # ==================================================================================================== PREDICT
...
# f = open(record_path+'lr_%.5f.txt'%params_learn.get('lr'), 'a+')
# f.write(str(evaluator.evaluate_acc())+'\n')
# f.write(str(evaluator.evaluate_acc_classwise())+'\n')
# f.write(str(evaluator.print_summary_eval())+'\n')
# f.close()
...
if params_ctrl.get('learn'):
    ...
    for epoch in range(params_learn.get('n_epochs')):
        ...
        loss = hist.history['loss']
        acc = hist.history['accuracy']
        f.write(str(epoch)+','+str(loss[0])+','+str(acc[0])+','+str(evaluator.mean_acc)+'\n')
        f.flush()
